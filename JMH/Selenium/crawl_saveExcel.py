...
def make_excel(user_name, keyword):
    ...
    sheet1.cell(row=1, column=8).value = 'parent_id'
    sheet1.cell(row=1, column=9).value = 'paths'
    sheet1.cell(row=1, column=10).value = 'keyword'
    sheet1.cell(row=1, column=11).value = 'sub_keyword'
    sheet1.cell(row=1, column=12).value = 'pageContents'
...
# 엑셀에 크롤링된 데이터 저장 함수
def crawl_saveExcel(user_name, user_email, curr_url, prev_url, pageList, relativeKeywordList, level, parent_id, paths, keyword, sub_keyword, pageContents, screenshot, tagged, nowTime):
    ...
    for idx, info_values in enumerate(crawl_info_values):
        # 페이지 리스트(4), 서브 키워드(10), 본문 요약(11)인 경우(리스트 형태)
        # 페이지 리스트(4)와 서브 키워드(10), 본문 요약(11)의 경우는 검색 페이지와 홈페이지인 경우 추출이 안될 수 있기 때문에
        # 빈 리스트를 고려해주어야 한다.
        if idx == 4 or idx == 10 or idx == 11:
            if info_values == []:
        ...
        # 사용자명, 이메일, 현재 페이지, 이전 페이지, 부모노드, 경로, 키워드, 태그, 추출 시간인 경우(문자열 형태)
        else:
            if info_values == []:
                sheet1.cell(row=excel_row, column=excel_column).value = ''
            else:
